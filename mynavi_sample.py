...
def set_driver(hidden_chrome: bool=False):
    ...
    '''
    USER_AGENT = "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_13_6) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/99.0.4844.51 Safari/537.36"
...
def main(is_option: bool = False, page_limit: int=5, hidden_chrome: bool=False):
    ...
    while True:
        # 会社名・タイトル箇所の全体を取得
        recruit_elms = driver.find_elements(by=By.CSS_SELECTOR, value=".cassetteRecruit__heading.cassetteUseFloat")
            
        for recruit_elm in recruit_elms:
            # 会社名のテキストを取得
            recruit_name = recruit_elm.find_element(by=By.CSS_SELECTOR, value=".cassetteRecruit__name").text
            # タイトルのテキストを取得
            recruit_title = recruit_elm.find_element(by=By.CSS_SELECTOR, value=".cassetteRecruit__copy.boxAdjust").text
            print(recruit_name, recruit_title)
        
            # DataFrameに対して辞書形式でデータを追加する
            df = df.append(
                {"会社名": recruit_name, 
                 "求人タイトル": recruit_title,
                 "項目C": ""}, 
                ignore_index=True)
